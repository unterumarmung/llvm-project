...
static bool isIntegerWideningViableForSlice(const Slice &S,
  ...
  // partition.
  if (IntrinsicInst *II = dyn_cast<IntrinsicInst>(U->getUser()); II && (II->isLifetimeStartOrEnd() || II->isDroppable())) 
    return true;
  ...
  bool visitLoadInst(LoadInst &LI) {
    ...
    if (VecTy) {
      ...
      if (auto *AITy = dyn_cast<IntegerType>(NewAllocaTy))
        if (auto *TITy = dyn_cast<IntegerType>(TargetTy); TITy && (AITy->getBitWidth() < TITy->getBitWidth()))
          {
...
SROA::rewritePartition(AllocaInst &AI, AllocaSlices &AS, Partition &P) {
  ...
  if (Promotable) {
    for (Use *U : AS.getDeadUsesIfPromotable()) {
      ...
      if ((OldInst) && (isInstructionTriviallyDead(OldInst)))
...
void SROA::clobberUse(Use &U) {
  ...
  // minimal.
  if (Instruction *OldI = dyn_cast<Instruction>(OldV); OldI && (isInstructionTriviallyDead(OldI)))
    {
